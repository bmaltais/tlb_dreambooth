...
def main():
            ...
            if args.train_text_encoder and global_step == args.stop_text_encoder_training and global_step >= 30:
              if accelerator.is_main_process:
                ...
                if os.path.exists(frz_dir):
                  shutil.rmtree(frz_dir) # subprocess.call('rm -r '+ frz_dir, shell=True)
                os.mkdir(frz_dir)
                pipeline = StableDiffusionPipeline.from_pretrained(
               ...
               if global_step < args.max_train_steps and global_step+1==i:
                  ...
                  if accelerator.is_main_process:
                     ...
                     if args.train_text_encoder and os.path.exists(frz_dir):
                        shutil.rmtree(save_dir+'/text_encoder') # subprocess.call('rm -r '+save_dir+'/text_encoder/*.*', shell=True)
                        shutil.copytree(frz_dir, save_dir+'/text_encoder') # subprocess.call('cp -f '+frz_dir +'/*.* '+ save_dir+'/text_encoder', shell=True)                     
                     ...
                     shutil.rmtree(save_dir) # subprocess.call('rm -r '+ save_dir, shell=True)
    ...
    if accelerator.is_main_process:
      if args.dump_only_text_encoder:
        ...
        txt_dir=args.output_dir + "/text_encoder_trained"
        shutil.rmtree(txt_dir) # subprocess.call('rm -r '+txt_dir, shell=True)
        ...
        pipeline.save_pretrained(args.output_dir)
        if args.train_text_encoder and os.path.exists(frz_dir):
           shutil.rmtree(args.output_dir+'/text_encoder') # os.remove(args.output_dir+'/text_encoder/*.*')
           shutil.copytree(frz_dir, args.output_dir+'/text_encoder') # subprocess.call('mv -f '+frz_dir +'/*.* '+ args.output_dir+'/text_encoder', shell=True) 
           shutil.rmtree(frz_dir) # subprocess.call('rm -r '+ frz_dir, shell=True) 
        if args.push_to_hub:
